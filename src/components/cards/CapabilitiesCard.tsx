  ...
  return (
    <BentoCard className="p-6 sm:p-8 h-full" delay={320}>
      <div className="flex items-center gap-3 mb-5">
        <span className="text-[10px] font-mono text-base/20 tracking-widest">06</span>
        <span className="w-6 h-px bg-base/10" />
        <span className="text-[10px] font-mono text-base/30 tracking-widest uppercase">{t.about.whatIBuild}</span>
      </div>
      ...
      <ul className="space-y-2.5">
        {capabilities.map((cap) => (
          <li key={cap.label} className="flex items-center gap-2.5 group">
            ...
            <span className="text-xs text-base/40 group-hover:text-base/60 transition-colors duration-200">
              {cap.label}
            </span>
          </li>
        ))}
